...
The gateway registers one approval UI for PR and stack efforts:
...
then re-checks every approved PR head and runs its merge node itself.
...
Verify that the workflow is served:
...
`pr-pipeline` must appear with `"hasUi":true`.
