...
export function modelFamily(model) {
  ...
  if (/\bclaude\b|anthropic/.test(value)) return "claude";
  return "other";
}

export function defaultModeForModel(model, configuredDefault = DEFAULT_MODE) {
  return modelFamily(model) === "gpt" ? "off" : configuredDefault;
}
...
export default function ponytailExtension(pi) {
  ...
  pi.on("session_start", async (_event, ctx) => {
    ...
    const familyDefault = defaultModeForModel(ctx?.model, configuredDefaultMode);
    currentMode = resolveSessionMode(entries, familyDefault);
    syncStatus(ctx);
    if (!getQuietStartup()) {
