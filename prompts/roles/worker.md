...
- A terminal report is only `done` with acceptance evidence or `failed` with the concrete cause, evidence, and next action. Use one paragraph or bullets.
...
- Treat every external human-visible message, including Slack and email, as a DRAFT for Tim unless Tim explicitly delegated the send.
