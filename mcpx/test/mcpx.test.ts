...
const NOISY_FIXTURE = path.resolve(import.meta.dir, "fixture-noisy-stdio-server.ts");
...
describe("mcpx stdio transport", () => {
	...
	test("survives non-JSON noise interleaved in the server's stdout", async () => {
		const noisy = serverSchema.parse({ transport: "stdio", command: "bun", args: [NOISY_FIXTURE] });
		const tools = await listTools(noisy);
		expect(tools.map(tool => tool.name)).toEqual(["survivor"]);
	}, 15_000);
});
...
describe("mcpx http transport", () => {
	...
	test("SSE stream with a malformed/keepalive data line still yields the valid response", async () => {
		const httpServer = Bun.serve({
			port: 0,
			async fetch(request) {
				const body = (await request.json()) as { id?: number; method?: string };
				...
				if (body.method === "tools/list") {
					const good = JSON.stringify({ jsonrpc: "2.0", id: body.id, result: { tools: [{ name: "pong" }] } });
					// A keepalive/empty data line and a garbage data line precede the
					// real event — the client must skip them, not throw.
		...
		try {
			const server = serverSchema.parse({ transport: "http", url: `http://127.0.0.1:${httpServer.port}/mcp` });
			const tools = await listTools(server);
			expect(tools.map(tool => tool.name)).toEqual(["pong"]);
		} finally {
			httpServer.stop(true);
		}
	}, 15_000);
