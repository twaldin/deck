...
*Post-mortem evidence base: **POSTMORTEM.md** (19-day firstmate operating record — 728 captain messages, 463 lane sessions, workaround census; 6-scout analysis, 2026-07-22). Deltas D-A…D-H accepted and folded into SPEC v0.3.*
...
- Deterministic-*ish* reliability via reusable workflows; flexibility via NL skills; messy-reality interpretation (GitHub merge queue MQ weirdness, cancelled CI) stays agent judgment.
...
| I12 | **Deck is invisible to lindy** (and any shared org): no GitHub App installations, no per-repo deploy keys, no org/repo webhooks, no branch-protection or repo-settings changes — no team-visible footprint of any kind. Deck operates exclusively through Tim's existing user-scoped credentials (personal PATs + personal SSH key) and local processes; to lindy it looks like Tim running his normal tools, nothing more. |
...
2. **Substrate v1** — manifests+tails, lifecycle tools (`report_progress`/`ask_tim`/`dispatch`/`park`) with conciseness-cap schemas (SPEC §4.4), wake router + watch-ci adapter, worktree primitive (treehouse verdict), minimal Deck TUI (list, cards, drilldown), **initial composed prompts: base rules + owner/worker role blocks carrying the three D-H doctrine rules (reviewer-deference, drafts-only comms, structural conciseness)**. *Exit: one real lindy PR driven end-to-end through an effort owner, AND the no-silent-drop receipt test (SPEC §4.5/D-A), AND the whole-system kill-9 + reboot recovery drill (SPEC §5.5.6d/D-C) all pass.*
3. **Workflow library** — pr-pipeline, watch-ci, fallout-watch as reusable defs; Smithers integration + deck primitive kit (adopt posture, spike validates); ui-review + browser vault + auth matrix (incl. lindy `lsid` takeover from robotim); skills overlay + content-hash dedup + full prompt composition (reviewer role block, codebase-map extraction); doctrine-mining pass; tool CLIs (slack/linear first, axi conventions).
...
| Merge authority is prompt convention, not enforced | Genuinely mechanized via two-tier **personal** credentials (SPEC §10, I12): agents get Tim's read-scoped personal PAT (no Contents:write → merge 403s; `gh pr merge --auto --squash` fails); push is Tim's personal SSH key (no per-repo deploy keys); branch protection is the team's existing config (Deck adds none). The real boundary: Tim's personal Contents:write credential in **macOS Keychain behind a per-merge biometric ACL**. No GitHub App (I12 — invisible to lindy). |
