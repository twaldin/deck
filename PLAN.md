...
One store, namespaced: `llm-accounts/` (3 Claude, 2–3 Codex, z.ai) + `tool-tokens/` (Slack/Notion/Google, populated later, schema day-one) + `gh/utility/deck-agent.pem` (GitHub **App** private key — the broker mints short-lived 1h installation tokens from it; not a PAT, which has no minting API) + repo SSH deploy keys. One refresh loop. **Claude module extracts omp's plan-limits mechanism (MIT source) — pi native OAuth and cli-proxy-api both burn extra-usage, so this is item #1.** Exposed to pi via `registerProvider`; rotation on 429/limit; usage events → board roster. The **`deck-merge` App key (Contents:write) is NOT in the store** — authority-tier, macOS Keychain, per-merge biometric release (SPEC §10/§13). Static-PAT fallback only if an org disallows App installation (SPEC §10). At cutover: retire VibeProxy + cli-proxy-api + all orphaned grants (pi's stale auth.json, plaintext `gho_` in `~/.claude/config.json`).
...
| Merge authority is prompt convention, not enforced | Genuinely mechanized via a **two-tier** model (SPEC §10/§13). Split transports stop the accidental path (workers get a read+PR-write API token + repo SSH deploy key; deploy keys can't call the REST API; branch protection blocks main). The real boundary: the `Contents: write` merge PAT lives in **macOS Keychain behind a per-merge Tim-biometric ACL** — a prompt-injected worker can trigger a prompt but cannot silently merge. (Honest correction: v0.2's "agents physically cannot merge" overclaimed under same-UID; the Keychain release is what actually satisfies I7.) |
