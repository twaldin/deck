...
One store, namespaced: `llm-accounts/` (3 Claude, 2–3 Codex, z.ai) + `tool-tokens/` (Slack/Notion/Google, populated later, schema day-one) + `gh/utility/` (agent API read-PATs + repo deploy keys — **utility tier**, SPEC §13). One refresh loop. **Claude module extracts omp's plan-limits mechanism (MIT source) — pi native OAuth and cli-proxy-api both burn extra-usage, so this is item #1.** Exposed to pi via `registerProvider`; rotation on 429/limit; usage events → board roster. The **merge PAT is NOT in the store** — it's an *authority-tier* credential held in macOS Keychain behind a Tim-biometric ACL (SPEC §10/§13). At cutover: retire VibeProxy + cli-proxy-api + all orphaned grants (pi's stale auth.json, plaintext `gho_` in `~/.claude/config.json`).
