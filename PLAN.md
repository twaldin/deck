...
| Merge authority enforced as prompt convention only | Mechanized: agent GH tokens carry **no merge scope**; a single `deck merge` gateway consumes a Tim-minted, head-SHA-bound, checks-green, single-use authorization (SPEC §10) — agents physically cannot merge a head Tim never stamped |
