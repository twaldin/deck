 ...
 * the GithubClient to resolve a PR that left the open search (SPEC of this tool:
 * a closed or unmerged state is NOT closed-without-landing until the
 * squash-commit search on the default branch comes back empty).
 */
...
/**
 * LANDING CHECK: a PR that fell out of the open search must be looked up
 * point-wise. A closed or unmerged state is resolved by searching the default
 * branch for the squash commit "(#N)"; only an empty search may be reported
 * as closed-without-landing.
 */
