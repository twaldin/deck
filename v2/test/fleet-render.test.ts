...
describe("fleet frame state", () => {
  test("counts unique live spawns", () => {
    expect(
      liveSpawnCount([
        { taskId: "a", runState: "running" },
        { taskId: "a", runState: "running" },
        { taskId: "b", runState: "finished" },
      ]),
    ).toBe(1);
  });
  test("asks once for a parked stamp and keeps resolved questions from reappearing", async () => {
    ...
    try {
      ...
      expect(fs.readFileSync(queue, "utf8")).toContain(
        '"questionKind":"stamp"',
      );
      const second = await buildFrame({
      ...
      const afterAnswer = await buildFrame({
        workflowCwd: directory,
        psRuns: [run],
      });
  ...
  test("stamp question includes durable context and counts historical generations", async () => {
    ...
    try {
      ...
      fs.writeFileSync(
        path.join(directory, "state", "ship", "old.input.json"),
      ...
      const question = JSON.parse(
        fs.readFileSync(process.env.DECK_QUESTIONS_FILE, "utf8"),
      );
      expect(question.id).toContain("acme/widgets:42:stamp");
      expect(question.prContext.prUrl).toBe(
  ...
  test("counts distinct live efforts and live spawned agents", async () => {
    ...
    try {
      ...
      for (const id of ["agent-a", "agent-b"]) {
        fs.writeFileSync(
          path.join(directory, "state", `${id}.status`),
          "working: live\n",
        );
        fs.writeFileSync(
          path.join(directory, "state", `${id}.meta`),
          `id=${id}\nrun_pid=${process.pid}\nkind=ship\nowner_system=deck\n`,
        );
      }
      const result = await buildFrame({
      ...
      expect(result.efforts?.map((effort) => effort.runId)).toEqual(
        expect.arrayContaining(["a", "b"]),
      );
      expect(result.tasks).toHaveLength(2);
      expect(result.tasks.map((task) => task.runState)).toEqual([
        "running",
        "running",
      ]);
  ...
  test("keeps every live generation, including same-repo PR recuts", async () => {
    ...
    try {
      ...
      for (const id of ["old", "new", "other-repo"]) {
        fs.writeFileSync(
          path.join(directory, "state", "ship", `${id}.input.json`),
          JSON.stringify({
            repo: id === "other-repo" ? "other/widgets" : "acme/widgets",
            existingPr: 7,
          }),
        );
      }
      const runs = [
        ...
        {
          id: "new",
          rootDir: `${directory}-new-worktree`,
          prNumber: 7,
          step: "watch-poll",
      ...
      expect(result.efforts?.map((effort) => effort.runId)).toEqual(
        expect.arrayContaining(["old", "new", "other-repo"]),
      );
...
describe("run collection", () => {
  test("uses one ps subprocess per tick and deduplicates concurrent collectors", async () => {
    ...
    const previousGateway = process.env.SMITHERS_GATEWAY_URL;
    let requests = 0;
    ...
    try {
      ...
      else process.env.SMITHERS_GATEWAY_URL = previousGateway;
      server.stop();
      fs.rmSync(directory, { recursive: true, force: true });
    }
  });
...
describe("buildFleetText", () => {
  ...
  test("long notes are truncated so rows stay width-safe", () => {
    const out = buildFleetText(
      frame({
        tasks: [task({ lastVerb: "working", lastNote: "x".repeat(300) })],
      }),
    );
...
describe("factory renderer", () => {
        ...
        efforts: [
          {
            ...
            ciState: "watching",
            reviewState: null,
            model: "implement deck/gpt-5.6-luna · review deck/claude-fable-5",
  ...
  test("windows factory rows for scrolling and clamps an overscroll", () => {
    ...
    const efforts = workflows.map((workflow, index) => ({
      identity: workflow.runId,
      ticket: `D-${index}`,
      prNumber: index,
      prTitle: null,
    ...
    const top = buildFactoryView(
      frame({ workflows, efforts }),
      PLAIN_FLEET_THEME,
      { maxBodyLines: 4, chrome: "bare" },
    );
    const bottom = buildFactoryView(
      frame({ workflows, efforts }),
      PLAIN_FLEET_THEME,
      { maxBodyLines: 4, scrollOffset: 9_999, chrome: "bare" },
    );
  ...
  test("status keeps completed work after factory omits it", () => {
    ...
    expect(renderStatus(frame({ workflows: [completed] }))).toContain(
      "completed",
    );
  });
  ...
  test("lists all resolved pipeline seats from its input", async () => {
    ...
    try {
      fs.mkdirSync(path.join(directory, "state", "ship"), { recursive: true });
      fs.writeFileSync(
        path.join(directory, "state", "ship", "run.input.json"),
        JSON.stringify({
          repo: "acme/widgets",
          models: {
            implementer: "deck/gpt-5.6-luna",
            watcher: "deck/gpt-5.6-sol",
            fallout: "deck/claude-fable-5",
          },
        }),
      );
      const result = await buildFrame({
      ...
      expect(result.efforts?.[0]?.model).toBe(
        "implement deck/gpt-5.6-luna · review deck/claude-fable-5 · watch deck/gpt-5.6-sol · fallout deck/claude-fable-5",
      );
    } finally {
      if (previousHome === undefined) delete process.env.DECK_V2_HOME;
      else process.env.DECK_V2_HOME = previousHome;
  ...
  test("renders broker usage by account, tier, and temperature", () => {
    const output = buildUsageText({
      reports: [
        {
          provider: "anthropic",
          metadata: { email: "captain@example.com" },
          limits: [
            {
              window: { id: "5h" },
              amount: { remainingFraction: 0.75 },
              status: "ok",
            },
            { label: "7d", amount: { usedFraction: 0.9 }, status: "warning" },
          ],
        },
      ],
    });
  ...
  test("renders exactly three width-safe lines with session and fleet facts", () => {
    const out = renderFooterLines(
      ...
      }),
      {
        ...
        cost: 0.123,
        usageLine: "claude 5h █████░ 91% · codex 7d █████░ 86%",
      },
      PLAIN_FLEET_THEME,
      140,
    );
...
describe("zombie workflow failures", () => {
  test("does not hide a failed run from unrelated landed chatter", () => {
    ...
    expect(actionableWorkflowFailures(frame({ workflows: [failed] }))).toEqual([
      failed,
    ]);
  });
  ...
  test("hides landed, superseded, and push-pr-null failures but keeps real failures", () => {
    ...
    expect(
      actionableWorkflowFailures(
        frame({ workflows: [real, landed, superseded, healthy, pushNull] }),
      ),
    ).toEqual([real]);
    expect(
      actionableWorkflowFailures(
  ...
  test("preflight refusal is hidden when a later healthy adopted run has the same PR", () => {
    ...
    expect(
      actionableWorkflowFailures(frame({ workflows: [refusal, successor] })),
    ).toEqual([]);
  });
});
...
describe("footer attention counts", () => {
  ...
  test("failed poll rows stay failed and terminal stamp rows do not ask", () => {
    ...
    expect(out).toContain("[failed]");
    expect(
      renderFooterLines(frame({ workflows: [cancelledStamp] }))[2],
    ).toContain("Nq 0");
  });
  ...
  test("theme colors attention counts", () => {
    ...
    expect(renderFooterLines(f, {}, marked)[2]).toBe(
      "<warning>Nq 0 · 0 efforts · 0 agents</warning>",
    );
  });
