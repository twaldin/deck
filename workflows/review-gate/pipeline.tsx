...
import { Approval, ContinueAsNew, Loop, Parallel, PiAgent, Sequence, Task, Workflow, Worktree, createSmithers } from "smithers-orchestrator";
import { z } from "zod";
...
export default smithers((ctx) => {
  ...
  const reviewPath = (pr: Pr) => <Sequence><Loop id={`gate-loop-${pr.number}`} until={ready(ctx.latest(outputs.review, `gate-review-${pr.number}`), ctx.latest(outputs.state, `gate-state-${pr.number}`))} maxIterations={rounds} onMaxReached="return-last"><Sequence>
    ...
    <Task id={`gate-report-${pr.number}`} output={outputs.report} retries={1}>{async () => { const r = ctx.latest(outputs.review, `gate-review-${pr.number}`); const blockers = r?.blockers ?? []; if (blockers.length === 0) return { round: r?.round ?? 0, prNumber: pr.number, blockers, posted: false, requestedChanges: false, summary: "clean round" }; const body = reviewComment(pr, blockers); const fingerprint = `blockers:${pr.number}:${r?.headSha ?? "unknown"}:${blockers.join("\\n")}`; let postedNow = posted[fingerprint] !== undefined; if (!postedNow && !dryRun) { await ghRun(cli, ["pr", "comment", String(pr.number), "--repo", input.repo, "--body", body], input.worktree); await ghRun(cli, ["pr", "review", String(pr.number), "--repo", input.repo, "--request-changes", "--body", body], input.worktree); posted[fingerprint] = iso(); saveJson(postedPath, posted); postedNow = true; } return { round: r?.round ?? 0, prNumber: pr.number, blockers, posted: postedNow, requestedChanges: postedNow, summary: body }; }}</Task>
  ...
  </Sequence></Loop><Task id={`gate-final-state-${pr.number}`} output={outputs.state} retries={1} dependsOn={[`gate-review-${pr.number}`, `gate-state-${pr.number}`]}>{async () => { const s = dryRun ? { mergeable: (fixtures.blockers ?? []).length === 0, ciGreen: (fixtures.blockers ?? []).length === 0, ciPending: false, mergeStateStatus: "CLEAN", headSha: "fixture-head", summary: "fixture final state" } : await state(cli, input.repo, pr.number); return { ...s, round: rounds, prNumber: pr.number }; }}</Task></Sequence>;
  const pollTask = async () => { if (dryRun && (fixtures.requested === false || pollCount >= polls || pollCount < (fixtures.pollCount ?? 0))) return []; if (rows.length > 0) await sleep(1000); if (dryRun) return discovered; const fresh = await requested(cli, input.repo, input.captainLogin); return [...new Map([...(rows.at(-1)?.prs ?? []), ...fresh].map((pr) => [pr.number, pr])).values()]; };
  ...
  const cleanCaptainOptions = ["Approve", "Hold", "Deny gate"];
  ...
  return <Workflow name="lindy-review-gate"><Sequence><Parallel maxConcurrency={1}>{reviewPrs.map((pr) => <Worktree id={`gate-review-worktree-${pr.number}`} path={pathFor(pr)} branch={pr.headRefName}><Sequence>{reviewPath(pr)}</Sequence></Worktree>)}</Parallel>{reviewPrs.map((pr) => <Sequence><Task id={`captain-question-${pr.number}`} output={outputs.captainQuestion} retries={1} dependsOn={[`gate-final-state-${pr.number}`]}>{async () => { const r = ctx.latest(outputs.review, `gate-review-${pr.number}`); const s = ctx.latest(outputs.state, `gate-final-state-${pr.number}`) ?? ctx.latest(outputs.state, `gate-state-${pr.number}`); const blockers = r?.blockers ?? []; const fresh = dryRun ? s : await state(cli, input.repo, pr.number); const current = fresh?.headSha === s?.headSha && fresh?.mergeable === true; const clean = ready(r, s) && current; const verdict = clean ? "approve" : "request-changes"; const comment = clean ? cleanComment(pr, r?.summary ?? "clean") : reviewComment(pr, blockers); if (pendingState(fresh)) return { queued: false, id: null, prNumber: pr.number, summary: "CI is still running; waiting for a settled result." }; if (s?.headSha === undefined || s.headSha === "") return { queued: false, id: null, prNumber: pr.number, summary: "head SHA unavailable" }; const id = `review-gate-pr-${pr.number}-${s.headSha}`; if (priorQuestionIds.has(id) || readQuestions(queueFile()).some((x) => x.id === id)) return { queued: false, id, prNumber: pr.number, summary: "question already queued" }; const event = askIfAbsent(queueFile(), { id, idScope: "global", question: clean ? `Captain review decision: PR #${pr.number}: ${pr.title}` : `Captain decision needed for blocked PR #${pr.number}: ${pr.title}`, prContext: { prUrl: pr.url, prRepo: input.repo, prNumber: pr.number, headSha: s?.headSha, prTitle: pr.title, draftedVerdict: verdict, draftedComment: comment, evidence: `Review: ${r?.summary ?? "not recorded"}\nState: ${s?.summary ?? "not recorded"}`, workflowDir: process.cwd(), workflowFile: fileURLToPath(import.meta.url) }, questionKind: clean ? "stamp" : "agent", origin: clean ? "fleet" : "review-gate", options: clean ? cleanCaptainOptions : ["Hold", "Close PR"], actions: clean ? ["stamp", "hold", "deny-gate"] : ["hold", "close-pr"], deliverAnswer: false, recommendation: clean ? "Approve only after reviewing the evidence." : "Hold or close this blocked PR.", sessionId: process.env.PI_SESSION_ID ?? "review-gate", cwd: input.worktree }); return { queued: true, id: event.id, prNumber: pr.number, summary: comment }; }}</Task>{dryRun ? <Task id={`review-approval-gate-${pr.number}`} output={outputs.reviewApproval}>{() => ({ approved: true, note: "bypassApprovals (dry-run test mode)", decidedBy: "bypass", decidedAt: iso() })}</Task> : <Approval id={`review-approval-gate-${pr.number}`} output={outputs.reviewApproval} onDeny="continue" request={{ title: `Captain review decision: PR #${pr.number} ${pr.title}`, summary: `URL: ${pr.url}\nThe captain decision is queued with the complete drafted verdict, comment, and evidence.` }} />} <Task id={`submit-review-${pr.number}`} retries={1} dependsOn={[`review-approval-gate-${pr.number}`]}>{async () => { const decision = ctx.latest(outputs.reviewApproval, `review-approval-gate-${pr.number}`); if (!shouldSubmitReview(decision)) return { submitted: false, reason: "captain denied" }; const r = ctx.latest(outputs.review, `gate-review-${pr.number}`); const blockers = r?.blockers ?? []; const verdict = blockers.length === 0 ? "approve" : "request-changes"; const marker = `submitted:${pr.number}:${r?.headSha ?? "unknown"}:${verdict}`; if (posted[marker] !== undefined) return { submitted: false, reason: "already submitted", verdict }; const args = [...reviewCommand(pr.number, input.repo, blockers.length === 0), "--body", blockers.length === 0 ? cleanComment(pr, r?.summary ?? "clean") : reviewComment(pr, blockers)]; if (!dryRun) { await ghRun(cli, args, input.worktree); posted[marker] = iso(); saveJson(postedPath, posted); } return { submitted: true, verdict }; }}</Task></Sequence>)}<ContinueAsNew state={{ ...input, pollCount, phase: "poll" }} /></Sequence></Workflow>;
});
