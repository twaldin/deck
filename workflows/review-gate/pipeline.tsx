...
function agent(model: string, noTools = false): PiAgent {
  return new PiAgent({ provider: "deck", model, timeoutMs: 30 * 60_000, thinking: "medium", noSession: true, ...(noTools ? { noTools: true } : {}), tools: noTools ? undefined : ["read", "grep", "edit", "write"] });
}
...
export default smithers((ctx) => {
  ...
  const reviewModel = new PiAgent({ provider: "deck", model: "gpt-5.6-sol", cwd: input.worktree, timeoutMs: 30 * 60_000, thinking: "medium", noSession: true, tools: ["read", "grep"] }); const fixModel = agent("gpt-5.6-luna"); const rebaseModel = agent("gpt-5.6-luna");
  void policy;
  ...
  const reviewPath = (pr: Pr) => {
    ...
    return <Sequence><Loop id={`gate-loop-${pr.number}`} until={gateReady(latest, checked)} maxIterations={rounds} onMaxReached="return-last"><Sequence>
      ...
      {latest && (!latest.approvable || latest.blockers.length > 0 || (checked && !checked.ciGreen)) && (fixed === undefined || fixed.round < latest.round || (checked && !checked.ciGreen)) ? <Worktree id={`gate-fix-worktree-${pr.number}`} path={isolatedWorktree(pr)} branch={pr.headRefName}><Task id={`gate-fix-${pr.number}`} output={outputs.fix} agent={dryRun ? undefined : fixModel} retries={1}>{dryRun ? () => ({ round: latest.round, prNumber: pr.number, dispatched: true, addressed: latest.blockers, summary: "fixture fix dispatched" }) : `Fix every finding from Sathira's Gate review for PR #${pr.number} (${pr.url}) only in the dedicated worktree ${isolatedWorktree(pr)}. ${failedChecks(checked)} First create or verify that worktree is checked out at PR head branch ${pr.headRefName ?? "the PR branch"}; never edit or push ${input.worktree}. Make the smallest safe changes. The only available tools are local file read/write tools; do not invoke GitHub, git, shell, approval, or network commands. Return JSON {"round":${latest.round},"prNumber":${pr.number},"dispatched":true,"addressed":string[],"summary":string}. Do not approve the PR.`}</Task></Worktree> : null}
      {latest && checked && (!checked.mergeable || !checked.ciGreen) ? <Worktree id={`gate-rebase-worktree-${pr.number}`} path={isolatedWorktree(pr)} branch={pr.headRefName}><Task id={`gate-rebase-${pr.number}`} output={outputs.rebase} agent={dryRun ? undefined : rebaseModel} retries={1}>{dryRun ? () => ({ round: latest.round, prNumber: pr.number, rebased: true, summary: "fixture rebase checked" }) : `Inspect PR #${pr.number} (${pr.url}) only in dedicated worktree ${isolatedWorktree(pr)}. Verify its branch is ${pr.headRefName ?? "the PR branch"} before any change. If it is behind or conflicting, rebase that branch onto the base branch and push only that branch. Never edit or push ${input.worktree}. The only available tools are local file read/write tools; do not invoke GitHub, git, shell, approval, or network commands. Return JSON {"round":${latest.round},"prNumber":${pr.number},"rebased":boolean,"summary":string}. Never approve the PR. If CI fails on a clean branch, report rebased:false so the fix task repairs CI.`}</Task></Worktree> : null}
    </Sequence></Loop><Task id={`gate-final-state-${pr.number}`} output={outputs.state} retries={1}>{async () => { const result = dryRun ? { mergeable: (fixtures.blockers ?? []).length === 0, ciGreen: (fixtures.blockers ?? []).length === 0, headSha: "fixture-head", summary: "fixture final state" } : await state(cli, input.repo, pr.number); const finalReview = (ctx.outputs.review ?? []).filter((x: any) => x.prNumber === pr.number).at(-1); const final = { ...result, round: reviews.filter((x) => x.prNumber === pr.number).length, prNumber: pr.number }; if (!gateReady(finalReview, final)) { if (!dryRun) askIfAbsent(queueFile(), { id: `review-gate-blocked-${pr.number}-${final.headSha}`, idScope: "global", question: `Review gate did not clear PR #${pr.number}: captain action required`, context: `${final.summary}. Review blockers: ${JSON.stringify(finalReview?.blockers ?? [])}`, questionKind: "stamp", urgency: "high", sessionId: process.env.PI_SESSION_ID ?? "review-gate", cwd: input.worktree }); throw new Error(`review gate unresolved for PR #${pr.number}: ${final.summary}`); } return final; }}</Task></Sequence>;
  };
  return <Workflow name="lindy-review-gate"><Parallel maxConcurrency={1}>
    ...
    {prs.map(reviewPath)}
    {prs.map((pr) => { const latest = (ctx.outputs.review ?? []).filter((x: any) => x.prNumber === pr.number).at(-1); const checked = (ctx.outputs.state ?? []).filter((x: any) => x.prNumber === pr.number).at(-1); const durable = openQuestions(queueFile()).some((x) => x.id === `review-gate-pr-${pr.number}-${latest?.headSha ?? checked?.headSha}`); return gateReady(latest, checked) && !durable ? <Task id={`captain-question-${pr.number}`} output={outputs.captainQuestion} retries={1}>{async () => { const review = latest!; const fresh = dryRun ? { mergeable: true, ciGreen: true, headSha: "fixture-head" } : await state(cli, input.repo, pr.number); if (!gateReady(review, fresh)) return { queued: false, id: null, prNumber: pr.number, summary: "head or gate state changed; review required" }; const summary = [`PR: ${pr.url}`, `Title: ${pr.title}`, `What it does: ${review.summary}`, `Gate findings: ${JSON.stringify(review.findings)}`, `CI + review state: verified mergeable and green at ${fresh.headSha}`, `Only the captain may approve it.`].join("\n"); if (dryRun) return { queued: true, id: "dry-run:captain-question", prNumber: pr.number, summary }; const event = askIfAbsent(queueFile(), { id: `review-gate-pr-${pr.number}-${fresh.headSha}`, idScope: "global", question: `Captain approval needed for PR #${pr.number}: ${pr.title}`, context: summary, questionKind: "stamp", urgency: "high", sessionId: process.env.PI_SESSION_ID ?? "review-gate", cwd: input.worktree }); return { queued: true, id: event.id, prNumber: pr.number, summary }; }}</Task> : null; })}
  </Parallel><ContinueAsNew state={{ trigger: "review-request-change", launcher: "review-gate/launch.ts" }} /></Workflow>;
});
