...
export default smithers((ctx) => {
  ...
  const discovered = dryRun && fixtures.requested !== false ? [{ number: fixtures.prNumber ?? 1, url: `https://github.com/${input.repo}/pull/${fixtures.prNumber ?? 1}`, title: fixtures.title ?? "fixture PR" }] : [];
  ...
  const reviewPrompt = (pr: Pr, round: number) => `You are Sathira's Gate review engine. Review PR #${pr.number} (${pr.url}) in ${input.repo} at ${input.worktree}. Use bash to fetch the PR head and inspect the exact PR diff and CI with git and gh before reviewing. You have read-only authority: do not edit, commit, or push. Approval is forbidden. NEVER run any GitHub approval command.
GitHub API mutations, git push, and write commands are forbidden. Load the exact skills .agent/skills/sathiras-gate and .agent/skills/thermo-nuclear-code-quality-review before reviewing. Review only this PR's actual diff and CI. Return JSON only: {"round":${round},"prNumber":${pr.number},"approvable":boolean,"blockers":string[],"findings":string[],"summary":string,"headSha":"the PR head SHA you reviewed"}. The workflow independently verifies mergeability and CI; do not claim those checks prove anything.`;
