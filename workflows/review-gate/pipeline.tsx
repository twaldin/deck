...
export default smithers((ctx) => {
  ...
  const reviewPath = (pr: Pr) => <Sequence><Loop id={`gate-loop-${pr.number}`} until={ready(ctx.latest(outputs.review, `gate-review-${pr.number}`), ctx.latest(outputs.state, `gate-state-${pr.number}`))} maxIterations={rounds} onMaxReached="return-last"><Sequence>
    ...
    <Task id={`gate-report-${pr.number}`} output={outputs.report} retries={1}>{async () => { const r = ctx.latest(outputs.review, `gate-review-${pr.number}`); const blockers = r?.blockers ?? []; if (blockers.length === 0) return { round: r?.round ?? 0, prNumber: pr.number, blockers, draftBody: "", draftFingerprint: "", posted: false, requestedChanges: false, summary: "clean round" }; const body = reviewComment(pr, blockers); const fingerprint = `blockers:${pr.number}:${r?.headSha ?? "unknown"}:${blockers.join("\\n")}`; let postedNow = posted[`comment:${fingerprint}`] !== undefined; if (!postedNow && !dryRun) { await ghRun(cli, ["pr", "comment", String(pr.number), "--repo", input.repo, "--body", body], input.worktree); posted[`comment:${fingerprint}`] = iso(); saveJson(postedPath, posted); postedNow = true; } return { round: r?.round ?? 0, prNumber: pr.number, blockers, draftBody: body, draftFingerprint: fingerprint, posted: postedNow, requestedChanges: false, summary: body }; }}</Task>
