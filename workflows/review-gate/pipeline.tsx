...
export default smithers((ctx) => {
  ...
  const reviewPrompt = (pr: Pr, round: number) => `You are Sathira's Gate review engine. Review PR #${pr.number} (${pr.url}) in the dedicated PR worktree ${isolatedWorktree(pr)}. The worktree must be checked out at ${pr.headRefName ?? "the PR head branch"}; verify the exact head SHA with gh before reviewing. Use bash to inspect only this PR's actual diff and CI. You have read-only authority: do not edit, commit, push, or approve. NEVER run any GitHub approve command. The repository supplies the gate rules inline in this prompt; do not assume unavailable skill files. Review only this PR. Return JSON only: {"round":${round},"prNumber":${pr.number},"approvable":boolean,"blockers":string[],"findings":string[],"summary":string,"headSha":"the PR head SHA you reviewed"}. The workflow independently verifies mergeability and CI.`;
