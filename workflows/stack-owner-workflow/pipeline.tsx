...
export default smithers((ctx) => {
  ...
  const implAgent = agent("gpt-5.6-luna"); const reviewAgent = agent("gpt-5.6-sol");
  const implementation = <Task id="implement-stack" output={outputs.implementation} agent={dryRun ? undefined : implAgent}>
    {dryRun ? () => ({ changed: true, prs: (input.fixtures?.prs ?? [1]).map((number) => ({ number, branch: input.branch })), summary: "dry-run implementation" }) : `Implement this prompt in ${input.worktree}: ${input.prompt}. You own the complete ordered PR stack. Split only when needed. Commit each branch and open no PR yet. Return JSON with changed, prs [{number,branch}] and summary.`}
  </Task>;
  ...
  const reviewLoop = <Loop id="adversarial-loop" maxIterations={maxRounds} onMaxReached="return-last" until={(ctx.latest(outputs.review, "adversarial-review")?.blockers?.length ?? 1) === 0}>
    <Sequence>{review}<Task id="fix-review" agent={dryRun ? undefined : agent("gpt-5.6-luna")}>
      {dryRun ? () => ({ fixed: true }) : `Fix every blocker in the latest adversarial review in ${input.worktree}. Run focused tests and commit. Do not open or merge PRs. If the same critical finding appears twice, stop and report it to the orchestrator.`}
    </Task></Sequence>
  </Loop>;
  const open = <Task id="open-stack" output={outputs.implementation} retries={1}>{async () => {
    const impl = ctx.latest(outputs.implementation, "implement-stack"); const prs = impl?.prs ?? [];
    if (dryRun) return { ...impl, summary: "dry-run PR stack opened" };
    for (const pr of prs) await execOrThrow(bunExec, [gh, "pr", "create", "--repo", input.repo, "--head", pr.branch, "--title", input.prompt.slice(0, 80), "--body", input.prompt], { cwd: input.worktree });
    if (prs.length) {
      ...
      await executeReviewerRequest({ explicit: input.github?.reviewers ?? [], exclude: [], denylist: [], max: 2 }, {
        fetchChangedFiles: () => fetchChangedFiles(context, prs[0].number), fetchCodeowners: () => fetchCodeowners(context), fetchRecentAuthors: (files) => fetchRecentAuthors(context, files), resolveLogin: (name) => resolveReviewerLogin(context, name), isCollaborator: (login) => isCollaborator(context, login), requestReviewers: (logins) => requestReviewers(context, logins), fetchRequestedReviewers: () => fetchRequestedReviewers(context, prs[0].number),
      });
    }
    return { ...impl, summary: "PR stack opened in dependency order" };
  }}</Task>;
  const poll = <Task id="poll-stack" output={outputs.poll} retries={2}>{async () => {
    ...
    const result = await pollStack(bunExec, input.repo, prs.map((p) => p.number), gh); return { signal: result.signal, reason: result.reason };
  }}</Task>;
  ...
  return <Workflow name="lindy-stack-owner"><Sequence>{implementation}{reviewLoop}{open}{watch}<Task id="done" output={outputs.result}>{() => ({ done: true, summary: input.profile === "lindy-full" ? "green stack parked for captain stamp" : "green stack ready for yolo merge" })}</Task></Sequence></Workflow>;
});
