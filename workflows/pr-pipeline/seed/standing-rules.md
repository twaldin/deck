...
prompts; the companion SETUP-CHECKLIST.md is TO-BUILD. Progressive disclosure:
open the cited source only when the topic goes deep. No secret values here.

Already-loaded deck files are NOT duplicated: merge/Linear traps live in
`~/.deck/data/lindy-domain.md`, prod rails in `lindy-ops.md`, pipeline SOP in
`lindy-pipeline.md`. This file adds what the firstmate archive holds that deck
files do not.

## 1. The "make PR" flow (captain's target, binding)

Captain says make PR → orch grills if shape is unclear → smithers pr-pipeline
(implement in worktree → adversarial review → push + PR → real reviewers → CI +
human approval) → captain stamp + per-PR merge word → land → deploy + fallout →
evidence-gated done.

- Grill/ideation is OUTSIDE the workflow; the workflow starts at "shape is done."
  [~/.deck/data/lindy-pipeline.md §Boundaries]
- Agents own the whole middle (build→review→rebase→reviewer chase→easy-fix
...
- Feedback routing: easy/correctness-class → fix + push instantly, answer thread,
...
- No unwatched PR ever: watch-ci-review armed on every open PR; keep PRs rebased
  on main (stale branches = wasted CI). [captain-pr-pipeline-spec.md §Standing orders]
- Never gate the stamp request on CI green — ask while CI runs; only real
  decisions block on the captain. [lindy-pipeline.md stage 6, ruling 2026-07-27]
- Validation/CI failure on already-authorized work is FIX-NOW: dispatch
  immediately; only the merge waits for the word. [~/.deck/data/lindy-learnings.md 2026-07-29]
- Kill-switch named in every PR brief, or "none + named break-signal" that the
  fallout watch then watches specifically. [~/firstmate/data/captain.md §PR pipeline]
- "Should this exist / product fit" check at inception AND before requesting
  review. [captain-pr-pipeline-spec.md]

## 2. Merge authority & autonomy

- lindy: captain's per-PR word, always. No blanket authority; "get it out the
  door" = prepare/chase, not merge. [~/firstmate/data/captain.md; ~/.deck/data/captain.md]
- deck + machine infra: YOLO on green. Destructive/irreversible/security still
  escalate. kalshi: YOLO + ~$300 loss cap. [~/.deck/data/captain.md §Project autonomy]
- Merge windows: no merges while captain is AFK overnight — prepare and
  batch-queue for morning. [~/firstmate/data/captain.md §Merge windows]
- Human stamp: bot/agent/AI-on-behalf reviews never satisfy the gate; never Ali
  as code stamp. Agents may dismiss a stale changes-requested once addressed;
  agents never approve. [captain.md; lindy-domain.md]
- Stale-review dismissal: a changes-requested from a reviewer who ignored the
...
- Scope-growth badge-riding: after approval, scope growth defaults to fresh
...
- Team-facing text: ASD-STE100, plain language, no fleet vocabulary ever.
...
- Agent attribution at the END; `-- tim's agent` on lindy only, not every repo.
  [~/.deck/data/lindy-learnings.md 2026-07-30 Q4]
- His Slack voice for drafts: all lowercase, casual shorthand (u, tysm, pls,
...
- Slack Connect channels (#team-engineering) reject API posts — plan for a
  human one-click send. One draft per channel limit. [~/firstmate/data/learnings.md]
- Never bare PR/ticket numbers — every mention carries what-it-is context and
  full https:// URLs. [~/firstmate/data/captain.md; ~/.deck/AGENTS.md]

## 5. Decisions

- Questions queue is THE decision surface; one decision at a time; each card is
...
- MAX-CRITICAL program mode (template for flagged criticals): every crewmate
  uncertainty bubbles to the captain via orch; vision-level questions asked
  BEFORE dispatch, not after. [~/firstmate/data/captain.md 2026-07-25]

## 6. Dispatch & workers

- One worker, one task; two workers never share a branch. Front-loaded
...
- Long CI/CD polls live in the ORCH process (pi-processes) or smithers nodes —
  never in ship-worker sleep loops; workers that poll-and-exit read as dead.
  [~/.deck/data/HANDOFF-2026-07-30.md §3.3]
- Orchestrator owns decomposition: split N independent streams yourself and
...
- Every ship task gets adversarial review pre-PR by a fresh-context reviewer
  from the opposite model family. Self-report is never the review.
  [~/.deck/AGENTS.md §4; ~/firstmate/data/captain.md]
- Repo-skills-first: before hand-solving auth/deploy/CI/prod-debugging, grep
  the repo's own skills (lindy has 70+ under `.agent/skills/`).
  [~/.deck/data/lindy-learnings.md 2026-07-28]
- Rate limits: never park — switch harness/account; park only when every lane
...
- Teardown: never discard unlanded work; discarding needs the captain's word
  for that specific task. [~/.deck/AGENTS.md §6]

## 7. Evidence standards

- Completion vocabulary is graded and distinct: patch-ready / applied /
...
- Landing truth = squash commit `(#N)` on main, never the merged flag
  (lands-and-closes, 3 repros). [lindy-domain.md trap 1]
- Verify CREATE-type calls by listing before retry; requests silently no-op or
  silently succeed. [~/.deck/data/learnings.md §Verifying]
- A regression test must go red on the old code — verify by reintroducing the
  bug (3 tests passed against broken code during the v2 build).
  [~/.deck/data/learnings.md §Verifying]
- Run receipts: every remote run start records {env, endpoint, runId, owner,
  startedAt, poller, terminal state} durably BEFORE polling.
  [lindy-learnings.md 2026-07-29 doctrine 6]
- Status grammar: lines start with the verb; append only actionable /
  authority / terminal events. Statuses are events, not state.
  [lindy-learnings.md doctrine 1–2; ~/.deck/data/learnings.md]
- Report outcomes faithfully; name what was NOT done alongside what was.
  [~/firstmate/data/captain-shared.md §Evidence]
- Never judge a number without knowing what computed it; pin dated model ids.
  [~/.deck/data/learnings.md]
- BLOCKED + 0 fails + 0 pending = check `behind_by` first; update-branch fixes
  the stuck "Expected" required check. [lindy-learnings.md 2026-07-28]

## 8. Prod-scale review gate (standing blind spot)

Changes fine in CI/local Mongo bite prod scale. Every new/changed Mongo
find/aggregate/write path in review requires index analysis against
multi-million-doc assumptions; call out load amplifiers (comparison arms,
per-completion writes) in PR risk; prefer flags for write-heavy
instrumentation; "tests green" is not prod-DB safety.
[~/.deck/data/lindy-learnings.md 2026-07-30 mongo huddle]

## 9. Lindy north star & eval doctrine (short form)

- Reliability self-improvement loop is #1; don't fix one-offs the loop should
...
- Eval fidelity: 1:1 with CURRENT prod — frozen task+user-state, prod-current
...
- Slack RTS results are redacted at storage — no eval can verify
...
- Scorer fixes are API/config-first (verify mutation path, snapshot, repair in
...
- e2e exercises the REAL LLM by design; safety = key isolation + per-key
  monitoring, never fake-LLM. e2e-imessage talks to LIVE PROD.
  [~/firstmate/data/captain.md; lindy-domain.md]

## 10. Linear & on-call

- Ticket creation contract: assignee=Tim, team AND project (default
...
- DONE IS TERMINAL (violated 2×, zero-tolerance). Follow-ups = new tickets.
  Never revert his bulk-Done sets. Full trap set: [lindy-domain.md §Linear]
- On-call retro is a required field and the Done gate: What happened / How we
...
- Never unilaterally cancel a teammate-originated ticket — verify createdBy AND
  attachments first. [~/firstmate/data/captain.md; lindy-domain.md]

## 11. Memory & homes

- Curated current-state files; rewrite and prune, never append forever. A fact
  with no evidence is a guess. [~/.deck/data/learnings.md header; ~/firstmate/data/captain.md]
- Project-intrinsic knowledge goes in that repo's AGENTS.md; fleet strategy
...
- Durable findings → reports under `data/`; scratch → worktree
  `claude-playground/` (dies with the worktree). Never mix.
  [~/firstmate/data/captain.md §Memory]
- Old firstmate home `~/firstmate` is read-only archive; never write it.
  ~/.deck must never be a git checkout. [lindy-learnings.md; deck-5 AGENTS.md]

## 12. Auth doctrine

- CLIs over MCPs: CLI auth is shared by every agent that shells out; MCP auth
  is per-harness pain. [~/firstmate/data/captain.md §Harnesses; learnings.md 2026-07-08]
- OAuth ToS split: bot OAuth accounts sanctioned for OpenAI/codex headless
...
- sentry + pup CLIs carry WRITE verbs — lanes stay read-only on them without
  explicit word. [~/firstmate/data/learnings.md 2026-07-08]
- When a credential seems dead, suspect extraction before the credential
