...
function clean(value: string): string {
	...
	// Drop whole clauses that are pure brief-ops residue
...
function summarize(text: string): string {
	...
	if (sentences.length === 0) {
		// last resort: cleaned blob without empty punctuation
		const blob = cleaned.replace(/^[\s.]+/, "").trim();
		return blob.length > 20
			? blob.slice(0, 480)
			: "This change updates eng-agent sandbox behavior.";
	}
	let out = "";
...
export function generatePullRequestDescription(input: PullRequestDescriptionInput): string {
	...
	const acceptance = input.brief.acceptanceCriteria
		...
		// drop residual "must approve" alone after captain strip
