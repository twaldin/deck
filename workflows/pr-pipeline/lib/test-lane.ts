...
export function testLaneCommand(command: string, lockPath = TEST_LANE_LOCK): string {
	...
	return `lock='${escapedLock}'; i=0; while :; do for slot in 0 1; do dir="$lock.$slot"; if mkdir "$dir" 2>/dev/null; then printf '%s\\n' "$$" > "$dir/pid"; trap 'rm -rf "$dir" 2>/dev/null || true' EXIT INT TERM; break 2; fi; reclaim="$dir.reclaim"; if mkdir "$reclaim" 2>/dev/null; then pid=$(cat "$dir/pid" 2>/dev/null || true); if [ -n "$pid" ] && ! kill -0 "$pid" 2>/dev/null; then rm -rf "$dir" 2>/dev/null || true; elif [ -z "$pid" ] && find "$dir" -mmin +1 -print -quit | grep -q .; then rm -rf "$dir" 2>/dev/null || true; fi; rmdir "$reclaim" 2>/dev/null || true; fi; done; i=$((i+1)); if [ "$i" -ge ${LANE_WAIT_SECONDS} ]; then echo 'TEST-LANE-PRESSURE: test lanes stayed busy' >&2; exit 75; fi; sleep 1; done; bash -lc '${escaped}'`;
}
