...
describe("pull request description", () => {
	...
	test("allows product behavior that names domain agents", () => {
		const body = generatePullRequestDescription(
			safeInput({
				summary:
					"Invoices are generated by the billing agent after approval. Reports are prepared by the review agent.",
			}),
		);
		expect(body).toContain("Invoices are generated by the billing agent");
		expect(body).toContain("Reports are prepared by the review agent");
	});
