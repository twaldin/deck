...
- **Split credentials (correct model — there is no "merge scope" to withhold).** GitHub gates `PUT /repos/{o}/{r}/pulls/{n}/merge` under **Contents: write**, identical to the authority an authenticated push needs, so a single token that can push can also merge. Enforcement therefore separates *transport*, not *scope*:
  - **Agent API credential = Tim's personal fine-grained PAT** (user-scoped to his GitHub account, repo-scoped to the repos he works on; scopes `Contents: read`, `Pull requests: write`, `Metadata: read`, + `Workflows: write` only if a task needs it). **No Contents:write ⇒ the merge REST endpoint 403s; `gh pr merge --auto --squash` fails for agents by design.** Held in the broker utility tier; `gh` for agents points at it. This PAT is *user-scoped* — invisible to the lindy org as a system (I12); it's just Tim's own credential, indistinguishable from him running `gh` himself. Tradeoff under I12: it's static (Tim rotates manually), not the short-lived App-installation tokens we'd prefer — Apps are banned (org-visible), so this is the cost of invisibility, accepted.
  - **Git push transport = Tim's existing personal SSH key** (already on his GitHub account, already his normal push mechanism). **No per-repo deploy keys** — those are repo-visible additions and violate I12. The SSH key is account-scoped, invisible at repo/org level. (The agent's API credential — the read-PAT — provably cannot push either, since it has no Contents:write; push is SSH-only, API is read-only. The split still holds.)
  - **Branch protection / rulesets**: rely on the **existing team-configured** protection on `main`/release branches; **Deck adds or modifies NO repo/org settings** (I12). If a repo lacks protection, Deck flags it as a risk and leans on the merge-gateway gate below — it does not quietly change repo settings.
  - **Merge gateway** = sole holder of Tim's personal `Contents: write` credential (his `gh auth token` OAuth token, or a dedicated write-PAT). Lives in **macOS Keychain under an ACL requiring Tim's biometric/password to release, per merge** — NOT in `broker/store.json`. Released only after (a) a valid Tim-minted single-use authorization (below) AND (b) the Keychain prompt fires. Both the agent PAT and the merge credential are Tim's *personal* credentials (invisible to lindy); the authority boundary is the **separate, higher-scope credential behind the Keychain prompt**, not org-level App separation. A prompt-injected worker can trigger a prompt (noisy/visible) but cannot silently merge.
  - **I12 invisibility summary for this section**: no GitHub App, no deploy keys, no branch-protection edits, no org/repo webhooks, no repo/org settings changes. Deck is a pure local consumer of Tim's personal GitHub identity — to lindy it looks like Tim running his normal tools, nothing more.
  - **Merge queue caveat**: agent merge-queue submissions must route through the gateway, which holds the write credential behind the Keychain release. Verified during Phase-3 onboarding per repo.
...
## External follow-ups
- Audit and update the remaining stack-management command references in the lindy repository.
- Audit and update the remaining stack-management command references in the user-skill repository.
...
3. ~~Router owns all owner processes as children vs per-session socket shim~~ **RESOLVED (Phase-2, forced by pi 0.73.0 architecture): router-owns.** pi has NO attach mechanism for a second process to reach a running session (source-verified); RPC mode is stdin-JSONL to the direct parent. Router spawns owners as `pi --mode rpc` children in their own process groups (inject via stdin, fence via pgid). D-D liveness probe: pi buffers the session JSONL until the first assistant message — file materialization on disk IS the first heartbeat (undocumented pi behavior, pinned by integration test; version-fragility noted).
