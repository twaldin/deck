...
After the version pin passes, preflight runs `ops/prime-patches.sh verify` when that repository verifier exists. A fingerprint mismatch fails with `PRIME_VERSION_MISMATCH`; an older checkout with no verifier remains compatible.
